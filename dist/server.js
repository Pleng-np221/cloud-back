...
app.post('/api/login', async (req, res) => {
    ...
    try {
        const result = await pool.query('SELECT * FROM main_userinfo WHERE email=$1', [email]);
    ...
    try {
        ...
        if (newPassword) {
            if (newPassword) {
                ...
                const match = await bcrypt_1.default.compare(currentPassword, dbUser.password);
                if (!match)
                    return res.status(400).json({ message: "Current password is incorrect" });
                finalPassword = await bcrypt_1.default.hash(newPassword, 10); // hash new password
            }
...
app.post("/api/products", uploadProduct.single("image"), async (req, res) => {
    try {
        ...
        // Insert product
        await pool.query(`INSERT INTO main_productdetail (
      ...
      VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9, NOW(), NOW())`, [newId, productName, price, description, size, material, category_id, stock, is_available]);
        // Insert image
        if (imagePath) {
            await pool.query(`INSERT INTO main_productimage (product_id, "imgURL") VALUES ($1, $2)`, [newId, imagePath]);
        }
...
app.put("/api/products/:id", uploadProduct.single("image"), async (req, res) => {
    try {
        ...
        // Update product
        await pool.query(`UPDATE main_productdetail
       SET "productName" = $1,
           price = $2,
           description = $3,
           size = $4,
       ...
       WHERE "productID" = $9`, [productName, price, description, size, material, category_id, stock, is_available === "true" || is_available === true, id]);
        ...
        if (req.file) {
            ...
            const imgExist = await pool.query(`SELECT * FROM main_productimage WHERE product_id = $1 LIMIT 1`, [id]);
            if (imgExist.rows.length > 0) {
                // Update existing image
                await pool.query(`UPDATE main_productimage SET "imgURL" = $1 WHERE product_id = $2`, [imagePath, id]);
            }
            else {
                // Insert new image
                await pool.query(`INSERT INTO main_productimage (product_id, "imgURL") VALUES ($1, $2)`, [id, imagePath]);
            }
        }
...
app.post("/api/cart/add", requireLogin, async (req, res) => {
    try {
        ...
        const existing = await pool.query(`SELECT * FROM main_customercart WHERE email_id=$1 AND product_id=$2 AND "customValue"=$3`, [email, productID, customValue]);
        if (existing.rows.length > 0) {
            await pool.query(`UPDATE main_customercart 
         SET quantities = quantities + $1 
         WHERE email_id=$2 AND product_id=$3 AND "customValue"=$4`, [quantities, email, productID, customValue]);
        }
        else {
            await pool.query(`INSERT INTO main_customercart (email_id, product_id, quantities, "customValue")
...
app.get("/api/cart/:email", requireLogin, async (req, res) => {
    try {
        ...
        const result = await pool.query(`SELECT cc.*, p."productName", p.price, pi."imgURL"
...
app.post("/api/cart/add-custom", uploadCustom.single("customImage"), requireLogin, async (req, res) => {
    try {
        ...
        const priceRes = await pool.query(`SELECT price FROM main_productdetail WHERE "productID"='KC_CUSTOM' LIMIT 1`);
        const customPrice = priceRes.rows[0]?.price ?? 0;
        const result = await pool.query(`INSERT INTO main_customproduct 
       ...
       RETURNING id`, [userEmail, profile, keyColor, textColor, customText, notes, imagePath, customPrice]);
        ...
        await pool.query(`INSERT INTO main_customercart 
...
app.put("/api/orders/:id", async (req, res) => {
    ...
    try {
        const result = await pool.query(`UPDATE main_order
       SET "orderStatus" = $1
       WHERE id = $2
...
app.post("/api/orders", requireLogin, async (req, res) => {
    ...
    try {
        ...
        // Insert order
        const orderResult = await client.query(`INSERT INTO main_order 
       ...
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8) RETURNING *`, [orderDate, orderStatus, totalPrice, name, phone, address, email_id, parcelStatus]);
        ...
        if (items && items.length > 0) {
            for (const item of items) {
                ...
                await client.query(`INSERT INTO main_orderitem 
        ...
        const cartIds = items
            ...
            .map((i) => i.id);
        if (cartIds.length > 0) {
            await client.query(`DELETE FROM main_customercart WHERE id = ANY($1)`, [cartIds]);
        }
...
app.post("/api/payment-details", requireLogin, upload.single("transferSlip"), async (req, res) => {
    try {
        ...
        const result = await pool.query(`INSERT INTO main_paymentdetail ("totalPrice", "transferSlip", "paymentDate", order_id)
...
app.get("/api/orderhistory", requireLogin, async (req, res) => {
    try {
...
app.put("/api/user/addresses/:id/default", requireLogin, async (req, res) => {
    ...
    try {
        ...
        await pool.query(`UPDATE main_useraddress
